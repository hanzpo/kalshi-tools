...
const KalshiLogoLarge = ({ color }: { color: string }) => (
  <svg
    width="120"
    height="46"
    viewBox="0 0 78 20"
...
const BANNER_PREVIEW_ID = 'banner-preview';

function ClassicBanner({ config }: BannerPreviewProps) {
  return (
    <div
      className="flex flex-col gap-5 bg-white p-[28px_32px] font-sans"
      style={{
        width: config.cardWidth || undefined,
        height: config.cardHeight || undefined,
...
function HorizontalBanner({ config }: BannerPreviewProps) {
  const isDark = config.variant === 'horizontal-dark';
  ...
  return (
    <div
      className="flex items-center gap-6 p-[20px_28px] font-sans"
      style={{
    ...
    >
      ...
      {/* Vertical divider */}
      <div
        className="h-[56px] w-px shrink-0"
        style={{ backgroundColor: dividerColor }}
      />

      {/* Right: Content */}
      <div className="flex min-w-0 flex-1 flex-col gap-1.5">
        <div className="text-[15px] font-semibold leading-tight" style={{ color: textColor }}>
          {config.title || 'Will this happen?'}
        </div>
        <div className="flex items-center gap-3">
          {config.image && (
            <img
              src={config.image}
              alt=""
              className="size-[32px] shrink-0 rounded object-cover"
            />
          )}
          ...
          {config.change && (
            <div className="flex items-center gap-0.5">
              ...
              />
              <span className="text-sm font-semibold" style={{ color: subtextColor }}>
                {config.change}
              </span>
            </div>
          )}
...
export function BannerPreview({ config }: BannerPreviewProps) {
  if (config.variant === 'horizontal' || config.variant === 'horizontal-dark') {
    return <HorizontalBanner config={config} />;
  }
  return <ClassicBanner config={config} />;
}

export { BANNER_PREVIEW_ID };
