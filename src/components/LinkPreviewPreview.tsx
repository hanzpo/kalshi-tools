          ...
          {config.marketType === 'binary' ? (
            <>
              ...
              <div className="link-preview-platform-logo">
                <svg width="50" height="13" viewBox="0 0 78 20" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ color: '#09C285' }}>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 5, right: 8, left: 15, bottom: 5 }}>
              ...
              <XAxis
                ...
                dataKey="time"
                stroke="transparent"
                tick={{ fill: secondaryTextColor, fontSize: 10, dx: 15 }}
                axisLine={false}
                tickLine={false}
                tickMargin={5}
                ticks={xAxisTicks}
              ...
              {(config.marketType === 'binary' || config.marketType === 'forecast') ? (
                <Line
                  ...
                  dot={(props: any) => {
                    ...
                    if (!isLast) return <circle key={`dot-${props.index}`} r={0} />;
                    return (
                      <circle
                        key={`dot-${props.index}`}
                        cx={props.cx}
                ...
                <>
                  {config.outcomes.map((outcome) => (
                    <Line
                      ...
                      dot={(props: any) => {
                        const isLast = props.index === data.length - 1;
                        if (!isLast) return <circle key={`dot-${outcome.id}-${props.index}`} r={0} />;
                        return (
                          <circle
                            key={`dot-${outcome.id}-${props.index}`}
                            cx={props.cx}
                            cy={props.cy}
                            r={4}
                            fill={outcome.color}
                      ...
                      activeDot={{ r: 4, fill: outcome.color }}
                      animationDuration={300}
                    />
                  ))}
                </>
